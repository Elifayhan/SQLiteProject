package com.elifayhan.sqliteproject;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try{
            ...
            //tablodan veri okuma. bunun için hücrelerin içini gezen bir cursora ihtiyacımız var.
            //Filtreleme
            //Cursor cursor= database.rawQuery("SELECT * FROM musicians WHERE name='James'", null);
            Cursor cursor= database.rawQuery("SELECT * FROM musicians", null);

            //sonu s ile bitenleri göstermek istersek
            //Cursor cursor= database.rawQuery("SELECT * FROM musicians WHERE name LIKE '%s'", null);
            //bu cursora hangi columnlara gideceğini söylemeliyiz.
            int nameIx= cursor.getColumnIndex("name");
            int ageIx= cursor.getColumnIndex("age");
            int idIx=cursor.getColumnIndex("id");
            //cursor bütün verileri gezerken biz de yazdıralım.
